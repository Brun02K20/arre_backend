...
import { categorias_services } from "../services/categorias.service.js";
...
router.get("/", async (req, res, next) => {
    try {
        const response = await categorias_services.getAll();
...
router.post("/", tokenExtractorMiddleware, async (req, res, next) => {
    try {
        const response = await categorias_services.createCategoria(req.body);
        return res.json(response);
    } catch (error) {
        next(error)
    }
...
router.put("/:id", tokenExtractorMiddleware, async (req, res, next) => {
    try {
        const response = await categorias_services.editCategoria(req.params.id, req.body);
        return res.json(response);
    } catch (error) {
        next(error)
    }
